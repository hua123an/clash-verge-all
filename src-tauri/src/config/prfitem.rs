...
impl PrfItem {
    ...
    pub async fn from_local(
        ...
        let normalized_file_data = match file_data {
            Some(file_data) => crate::config::subscription::normalize_subscription_text(&file_data)?
                .yaml
                .into(),
            None => tmpl::ITEM_LOCAL.into(),
        };
        ...
        Ok(Self {
            ...
            file_data: Some(normalized_file_data),
        })
    }
    ...
    pub async fn from_url(
        ...
        let inline_source = crate::config::subscription::looks_like_inline_source(url);
        let is_http_source = url.starts_with("http://") || url.starts_with("https://");

        if inline_source && !is_http_source {
            let normalized = crate::config::subscription::normalize_inline_source(url)?;
            let local_name = name
                .cloned()
                .or(normalized.suggested_name.map(Into::into))
                .unwrap_or_else(|| "Imported Subscription".into());
            let local_desc = desc.cloned().unwrap_or_default();
            return Self::from_local(local_name, local_desc, Some(normalized.yaml.into()), option).await;
        }
            ...
            .await
        {
            ...
            Err(e) => {
                if inline_source {
                    let normalized = crate::config::subscription::normalize_inline_source(url.as_str())?;
                    let local_name = name
                        .cloned()
                        .or(normalized.suggested_name.map(|value| String::from(value.as_str())))
                        .unwrap_or_else(|| "Imported Subscription".into());
                    let local_desc = desc.cloned().unwrap_or_default();
                    return Self::from_local(local_name, local_desc, Some(normalized.yaml.into()), option).await;
                }
        ...
        let filename: Option<String> = match header.get("Content-Disposition") {
            ...
            None => Some(
                crate::utils::help::get_last_part_and_decode(url.as_str())
        ...
        let raw_data = raw_data.trim_start_matches('\u{feff}');
        let normalized = match crate::config::subscription::normalize_subscription_text(raw_data) {
            Ok(normalized) => normalized,
            Err(_) if inline_source => {
                let normalized = crate::config::subscription::normalize_inline_source(url.as_str())?;
                let local_name = name
                    .cloned()
                    .or(normalized.suggested_name.map(Into::into))
        ...
        let name: String = name.map(|s| s.to_owned()).unwrap_or_else(|| {
            normalized
                .suggested_name
                .clone()
                .map(|value| String::from(value.as_str()))

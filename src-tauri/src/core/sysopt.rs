...
fn is_missing_default_network_interface_error(message: &str) -> bool {
...
impl Sysopt {
    ...
    pub async fn update_sysproxy(&self) -> Result<()> {
        ...
        let apply_result = tokio::task::spawn_blocking(move || -> Result<()> {
        ...
        if let Err(err) = apply_result {
            ...
            if is_missing_default_network_interface_error(&message) {
                logging!(
                    warn,
                    Type::Core,
                    "跳过系统代理应用：当前没有默认网卡可写入代理配置: {}",
                    message
                );
                return Ok(());
            }

...
use crate::{
    config::{
        ...
        profiles::{
            profiles_append_item_with_filedata_safe, profiles_delete_item_safe, profiles_draft_update_item_safe,
...
fn merge_profile_patch(mut base: PrfItem, patch: &PrfItem) -> PrfItem {
    if patch.itype.is_some() {
        base.itype = patch.itype.clone();
    }
    if patch.name.is_some() {
        base.name = patch.name.clone();
    }
    if patch.file.is_some() {
        base.file = patch.file.clone();
    }
    if patch.desc.is_some() {
        base.desc = patch.desc.clone();
    }
    if patch.url.is_some() {
        base.url = patch.url.clone();
    }
    if patch.selected.is_some() {
        base.selected = patch.selected.clone();
    }
    ...
    if patch.option.is_some() {
        base.option = patch.option.clone();
    }
    if patch.home.is_some() {
        base.home = patch.home.clone();
    }
    base
}

fn is_remote_source_option_patch(option: Option<&PrfOption>) -> bool {
    option.is_some_and(|option| {
        option.user_agent.is_some()
            || option.with_proxy.is_some()
            || option.self_proxy.is_some()
            || option.timeout_seconds.is_some()
            || option.danger_accept_invalid_certs.is_some()
    })
}

fn should_rebuild_profile_source(old_profile: &PrfItem, patch: &PrfItem) -> bool {
    old_profile.itype.as_deref() == Some("remote")
        && (patch.url.is_some() || patch.itype.is_some() || is_remote_source_option_patch(patch.option.as_ref()))
}
...
pub async fn patch_profile(index: String, profile: PrfItem) -> CmdResult {
    ...
    let old_profile = profiles.latest_arc().get_item(&index).stringify_err()?.clone();
    let merged_profile = merge_profile_patch(old_profile.clone(), &profile);

    let old_interval = old_profile.option.as_ref().and_then(|o| o.update_interval);
    let new_interval = merged_profile.option.as_ref().and_then(|o| o.update_interval);
    let old_allow_auto_update = old_profile.option.as_ref().and_then(|o| o.allow_auto_update);
    let new_allow_auto_update = merged_profile.option.as_ref().and_then(|o| o.allow_auto_update);
    let should_refresh_timer = (old_interval != new_interval) || (old_allow_auto_update != new_allow_auto_update);

    if should_rebuild_profile_source(&old_profile, &profile) {
        let profile_url = merged_profile
            .url
            .as_deref()
            .ok_or("profile url is missing after merge")?;

        let mut rebuilt_profile = PrfItem::from_url(
            profile_url,
            merged_profile.name.as_ref(),
            merged_profile.desc.as_ref(),
            merged_profile.option.as_ref(),
        )
        .await
        .stringify_err()?;

        rebuilt_profile.uid = old_profile.uid.clone();
        rebuilt_profile.file = old_profile.file.clone();
        rebuilt_profile.selected = merged_profile.selected.clone();

        profiles_patch_item_safe(&index, &rebuilt_profile)
            .await
            .stringify_err()?;
        profiles_draft_update_item_safe(&index, &mut rebuilt_profile)
            .await
            .stringify_err()?;
    } else {
...
#[cfg(test)]
mod tests {
    use super::{merge_profile_patch, should_rebuild_profile_source};
    use crate::config::{PrfItem, PrfOption};
    use smartstring::alias::String;

    fn remote_profile() -> PrfItem {
        PrfItem {
            uid: Some("Rtest".into()),
            itype: Some("remote".into()),
            name: Some("Remote".into()),
            file: Some("Rtest.yaml".into()),
    ...
    #[test]
    fn merge_profile_patch_preserves_old_fields_when_patch_is_partial() {
        let base = remote_profile();
        let patch = PrfItem {
            desc: Some("new description".into()),
            ..Default::default()
        };

        let merged = merge_profile_patch(base.clone(), &patch);

        assert_eq!(merged.url, base.url);
        assert_eq!(merged.file, base.file);
        assert_eq!(merged.desc, Some(String::from("new description")));
        assert_eq!(
            merged.option.as_ref().and_then(|option| option.merge.clone()),
            base.option.as_ref().and_then(|option| option.merge.clone())
        );
    }

    #[test]
    fn remote_url_change_triggers_rebuild() {
        let base = remote_profile();
        let patch = PrfItem {
            url: Some("vmess://payload".into()),
            ..Default::default()
        };

        assert!(should_rebuild_profile_source(&base, &patch));
    }

    #[test]
    fn selected_only_patch_does_not_trigger_rebuild() {
        let base = remote_profile();
        let patch = PrfItem {
            selected: Some(vec![]),
            ..Default::default()
    ...
    #[test]
    fn remote_fetch_option_patch_triggers_rebuild() {
        let base = remote_profile();
        let patch = PrfItem {
            option: Some(PrfOption {

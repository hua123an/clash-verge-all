...
export async function getSystemProxy() {
  try {
    ...
    console.warn('[API] get_sys_proxy 调用失败，回退为空配置:', error)
    return {
      enable: false,
      server: '',
      bypass: '',
...
export async function getAutotemProxy() {
  try {
    ...
    console.warn('[API] get_auto_proxy 调用失败，回退为空配置:', error)
    return {
      enable: false,
      url: '',
    }
...
export const isServiceAvailable = async () => {
  try {
    ...
    console.warn('Service check failed, fallback to unavailable:', error)
    return false
  }
}

...
const normalizeSysproxyError = (error: unknown) => {
  const message =
    error instanceof Error ? error.message : String(error ?? '')

  if (message.includes('failed to interact with SCPreferences')) {
    return 'settings.modals.sysproxy.messages.permissionDenied'
  }

  if (message.includes('failed to interact with SCDynamicStore')) {
    return 'settings.modals.sysproxy.messages.dynamicStoreFailed'
  }
...
export const SysproxyViewer = forwardRef<DialogRef>((props, ref) => {
  ...
  useEffect(() => {
    ...
    if (!enabled || !mixedPort || mixedPort === prevMixedPortRef.current) {
      return
    }
    ...
    const updateProxy = async () => {
      try {
        ...
        showNotice.error(normalizeSysproxyError(err))
      }
    }

...
const SUPPORTED_IMPORT_FILE_EXTENSIONS = ['.yaml', '.yml', '.json', '.conf', '.txt']
...
const isSupportedImportFile = (filePath: string) => {
  const lower = filePath.toLowerCase()
  return SUPPORTED_IMPORT_FILE_EXTENSIONS.some((ext) => lower.endsWith(ext))
}

const getImportSourceKind = (value: string) => {
  ...
  const looksLikeRemoteSource = REMOTE_IMPORT_SCHEME_RE.test(firstLine)
  ...
  return looksLikeRemoteSource && isMultiLine ? 'multi-link' : 'single-link'
}
...
const ProfilePage = () => {
  ...
  const importSourceKind = getImportSourceKind(url)
  const importHelperText =
    importSourceKind === 'config'
      ? 'Config text is imported directly. Use Ctrl/Cmd+Enter to start.'
      : `${t('profiles.modals.proxiesEditor.placeholders.multiUri')} Ctrl/Cmd+Enter to import.`
    ...
    >
      ...
      >
        ...
        <Button
          ...
          variant="contained"
          size="small"
          sx={{ borderRadius: '6px', alignSelf: 'flex-start' }}
          onClick={onImport}
        >
